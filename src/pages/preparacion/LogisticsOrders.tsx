...
} from "../../modules/logistics/components/CreateShiftDialog";
import type { DriverShift } from "../../modules/logistics/types/shift.types";
...
export default function LogisticsOrders() {
  ...
  const [shiftOrderIds, setShiftOrderIds] = useState<number[]>([]);
  const [error, setError] = useState<string | null>(null);
  const [refreshing, setRefreshing] = useState(false);
  ...
  const openShiftDialogForOrders = useCallback(
    (orderIds: number[]) => {
      const validIds = [...new Set(orderIds.map(Number))].filter(
        (id) => Number.isInteger(id) && id > 0,
      );

      if (!validIds.length) {
        setError("Seleccioná al menos un pedido listo para reparto.");
        return;
      }

      const selected = orders.filter((order) => validIds.includes(order.id));
      const notConfigured = selected.filter(
        (order) => !isConfiguredForDelivery(order),
      );

      if (notConfigured.length > 0) {
        setError(
          `Antes de armar la jornada confirmá fecha y dirección de los pedidos: ${notConfigured
            .map((order) => `#${order.id}`)
            .join(", ")}.`,
        );
        return;
  ...
  const handleShiftCreated = useCallback(
    async (shift: DriverShift, result: CreateShiftResult) => {
      ...
      if (result.failedOrderIds.length > 0) {
        setError(
          `La jornada #${shift.id} fue creada, pero no pudieron incorporarse estos pedidos: ${result.failedOrderIds
            .map((orderId) => `#${orderId}`)
            .join(", ")}. Revisá la jornada antes de iniciarla.`,
        );
        return;
      }
  ...
  const columns: GridColDef<Order>[] = useMemo(
        ...
        renderCell: (params) => (
          ...
          >
            ...
            <Button
              ...
              onClick={() => openShiftDialogForOrders([params.row.id])}
            >
              Armar jornada
            </Button>
          </Stack>
    ...
    [isMobile, openShiftDialogForOrders],
  );
                ...
                <Stack spacing={1.2}>
                  ...
                  <Stack spacing={1}>
                    ...
                    <Button
                      ...
                      fullWidth
                      disabled={!configured}
                      onClick={() => openShiftDialogForOrders([order.id])}
                    >
                      Armar jornada
                    </Button>
                  </Stack>
  ...
  return (
    <Container maxWidth="xl" sx={{ py: { xs: 1, md: 0.5 } }}>
          ...
          <Stack spacing={2}>
            ...
            >
              <Stack spacing={0.5}>
                ...
                <Typography variant="body2" color="text.secondary">
                  Listos para jornada: {selectedConfiguredCount}
                </Typography>
              </Stack>
            ...
            selectedConfiguredCount !== selectionModel.ids.size ? (
              <Alert severity="warning" sx={{ borderRadius: 3 }}>
                Algunos pedidos seleccionados todavía no tienen fecha o
                dirección de entrega confirmadas. Confirmalos antes de armar la
                jornada.
              </Alert>
            ) : null}
          </Stack>
      ...
      <CreateShiftDialog
        ...
        orderIds={shiftOrderIds}
        onClose={() => {
          setCreateShiftOpen(false);
          setShiftOrderIds([]);
        }}
        onSuccess={(createdShift, result) =>
          void handleShiftCreated(createdShift, result)
        }
      />
