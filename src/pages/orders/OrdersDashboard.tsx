...
const getRelevantDelivery = (order: Order): OrderDelivery | null => {
  const deliveries = [...(order.deliveries ?? [])];

  if (!deliveries.length) return null;

  const timestamp = (delivery: OrderDelivery) =>
    new Date(
      delivery.deliveredAt ?? delivery.attemptedAt ?? delivery.assignedAt,
    ).getTime();

  /*
   * Para un pedido completamente entregado se prioriza el intento
   * cuyo resultado fue DELIVERED. Para otros estados se utiliza
   * el último intento registrado.
   */
  const candidates =
    order.status === "DELIVERED"
      ? deliveries.filter(
          (delivery) =>
            delivery.resultStatus === "DELIVERED" ||
            delivery.deliveredAt != null,
        )
      : deliveries.filter(
          (delivery) =>
            delivery.resultStatus === order.status ||
            delivery.attemptedAt != null,
        );

  return (candidates.length ? candidates : deliveries).sort(
    (a, b) => timestamp(b) - timestamp(a),
  )[0];
};

const getDeliveryActivityLabel = (status: string) => {
  switch (status) {
    case "DELIVERED":
      return "Entregado por";
    case "PARTIAL_DELIVERED":
      return "Entrega parcial realizada por";
    case "RESCHEDULED":
      return "Reprogramado por";
    case "NOT_DELIVERED":
      return "Intento registrado por";
    case "IN_DELIVERY":
    case "ASSIGNED":
      return "Chofer asignado";
    default:
      return "Responsable de reparto";
  }
};
    ...
    <Box p={4} bgcolor="#f4f6f8" minHeight="100vh">
      ...
      <Stack spacing={2}>
        {filteredOrders.map((order) => {
          ...
          const hasGps =
            order.client?.latitude != null && order.client?.longitude != null;
          const relevantDelivery = getRelevantDelivery(order);
          const deliveryActivityAt =
            relevantDelivery?.deliveredAt ??
            relevantDelivery?.attemptedAt ??
            relevantDelivery?.assignedAt ??
            order.delivered_at ??
            order.last_delivery_attempt_at ??
            null;
                  ...
                  <Stack spacing={0.6}>
                    ...
                    {relevantDelivery?.deliveryUser && (
                      <Typography
                        ...
                        sx={{
                          fontWeight: order.status === "DELIVERED" ? 700 : 400,
                        }}
                      >
                        {getDeliveryActivityLabel(order.status)}:{" "}
                        {relevantDelivery.deliveryUser.full_name ||
                          relevantDelivery.deliveryUser.email ||
                          `Usuario #${relevantDelivery.deliveryUser.id}`}
                        {deliveryActivityAt
                          ? ` · ${formatDateTimeAR(deliveryActivityAt)} hs`
                          : ""}
                      </Typography>
                    )}
                  </Stack>
                  ...
                  <Paper
                    ...
                    sx={{
                      ...
                      borderLeft: `4px solid ${
                        order.status === "DELIVERED" ? "#2e7d32" : "#1976d2"
                      }`,
                      backgroundColor:
                        order.status === "DELIVERED" ? "#f1f8e9" : "#e3f2fd",
                    }}
                  >
                    <Stack direction="row" spacing={1} alignItems="flex-start">
                      ...
                      <Stack spacing={0.35}>
                        ...
                        <Typography variant="caption" color="text.secondary">
                          Intento #{relevantDelivery.attemptNumber}
                          {relevantDelivery.resultStatus
                            ? ` · Resultado: ${
                                STATUS[relevantDelivery.resultStatus]?.label ||
                                relevantDelivery.resultStatus
                              }`
                            : " · Asignación activa"}
                        </Typography>

                        {relevantDelivery.deliveryNotes?.trim() && (
                          <Typography variant="caption" color="text.secondary">
                            Observación: {relevantDelivery.deliveryNotes}
                          </Typography>
                        )}
                      </Stack>
                    </Stack>
