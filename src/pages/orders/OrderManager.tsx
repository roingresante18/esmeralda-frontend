...
export default function OrderManager({
  ...
  /*
   * Link público generado por el backend al confirmar el pedido.
   * Se utiliza en el comprobante y en el mensaje de WhatsApp.
   */
  const [confirmedTrackingUrl, setConfirmedTrackingUrl] = useState<
    string | null
  >(null);
  const [clientForm, setClientForm] = useState<ClientFormState>({
    name: "",
  ...
  const exportConfirmationToWhatsApp = async (trackingUrl?: string | null) => {
    if (!confirmationReceiptRef.current) return;
    ...
    if (phone) {
      ...
      const text =
        ...
        `Te comparto el comprobante con los detalles de entrega.` +
        trackingText;
    ...
    >
      ...
      <ConfirmOrderDialog
        ...
        onConfirm={async ({ payment, shouldSaveClientGps }) => {
          ...
          const confirmationResult = await confirmOrder(payment, {
            shouldSaveClientGps,
          });
          ...
          const trackingUrl = confirmationResult.trackingUrl;

          setConfirmedTrackingUrl(trackingUrl);

          setTimeout(async () => {
            await exportConfirmationToWhatsApp(trackingUrl);
            clearAll();
          }, 400);
        }}
      />
